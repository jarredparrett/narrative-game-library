...
def _model_observation(
    ...
    *,
    ...
    """Create a bounded role-visible snapshot or stateful turn delta."""
...
def _policy_guidance(role: str, observation: Mapping[str, Any]) -> list[str]:
    """Make protocol-critical choices salient without scripting the policy."""
    remaining = int(observation["remaining_steps"])
    if role == "host":
        return [
            "The host acts only once per complete seat cycle, so treat remaining_steps as a hard budget.",
            "Open a created session immediately.",
            "Advance phases promptly; make sure a seat reaches the resolution phase with at least one turn left.",
            "When evidence has been requested, disclose an exact authorized resource ID if the budget permits.",
            "Do not broadcast when advancing or disclosing is the higher-value legal action.",
            f"There are {remaining} total arena actions left.",
        ]
    return [
        "inspect_evidence accepts only an ID currently listed in game.resources.",
        "request_evidence accepts only an exact ID from requestable_resources; narrative evidence IDs in dossier prose are not resource IDs.",
        "In the resolution phase, prioritize submit_resolution with exact visible hypothesis and proof-path IDs.",
        "Use say to exchange bounded facts and update_character_state to record genuine objective progress.",
        f"There are {remaining} total arena actions left.",
    ]
